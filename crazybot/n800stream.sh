...
while bin/true; do gst-launch-0.10 v4l2src ! video/x-raw-yuv,width=$WIDTH,height=$HEIGHT,framerate=\(fraction\)$FPS/1 ! hantro4200enc stream-type=1 profile-and-level=245 bit-rate=515 ! video/mpeg,framerate=\(fraction\)$FPS/1 ! tcpserversink protocol=gdp port=$PORT; done
